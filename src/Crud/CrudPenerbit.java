...
public class CrudPenerbit {
    ...
    public void simpan(String nama) {
        try {
            String sql = "INSERT INTO penerbit (nama) VALUES (?)";
    ...
    public void update(int id, String nama) {
        try {
            String sql = "UPDATE penerbit SET nama=? WHERE id=?";
            PreparedStatement pst = conn.prepareStatement(sql);
            pst.setString(1, nama);
            pst.setInt(2, id);
            pst.executeUpdate();
    ...
    public void hapus(int id) {
        try {
            String sql = "DELETE FROM penerbit WHERE id=?";
            PreparedStatement pst = conn.prepareStatement(sql);
            pst.setInt(1, id);
            pst.executeUpdate();
        } catch (SQLException e) {
